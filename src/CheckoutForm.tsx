...
const CheckoutForm: React.FC = () => {
  ...
  // Login function to authenticate and get token
  const handleLogin = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    try {
      const response = await fetch('https://fm-dev-box.flip.id/big_web_api/v1/user/authenticate', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        setAuthToken(data.token); // Assuming the token is in `data.token`
        setIsLoggedIn(true);
  ...
  // API integration inside handleSubmit using the auth token
